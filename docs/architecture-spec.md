...
| **Input Monitoring** | Observe the global hotkey with a listen-only event tap | Phase 1 |
| **Accessibility** | Capture the focused field and insert text into other applications | Phase 3 |
...
| --- | --- | --- |
| A listen-only `CGEventTap` requires Input Monitoring trust, but creation can still return non-nil without it; taps can also be silently disabled | Hotkey appears installed but delivers nothing, or stops working with no visible cause | Gate health on `CGPreflightListenEventAccess`, recreate stale pre-permission taps on activation, observe `kCGEventTapDisabledByTimeout` / `…ByUserInput`, and expose live tap health in UI |
